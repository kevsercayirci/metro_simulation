...
class MetroAgi:
 ...
 def en_hizli_rota_bul(self, baslangic_id: str, hedef_id: str) -> Optional[Tuple[List[Istasyon], int]]:
        ...
        mesafeler = {istasyon: float('inf') for istasyon in self.istasyonlar.values()}
        ...
        while heap:
            mevcut_sure, mevcut = heapq.heappop(heap)
            
            if mevcut == hedef:
                rota = []
                while mevcut in onceki:
                    rota.append(mevcut)
                    mevcut = onceki[mevcut]
                rota.append(baslangic)
                rota.reverse()
                return rota, mesafeler[hedef]
            
            for komsu, sure in mevcut.komsular:
                yeni_sure = mevcut_sure + sure
                if yeni_sure < mesafeler[komsu]:
                    mesafeler[komsu] = yeni_sure
                    onceki[komsu] = mevcut
                    heapq.heappush(heap, (yeni_sure, komsu))
